...
if (mysqli_num_rows($result) > 0) {
	...
	$query  = "SELECT laps.sessionID,tracks.name,laps.time,vehicles.name FROM laps,tracks,vehicles WHERE laps.trackID = tracks.id AND laps.carID = vehicles.id AND laps.userID = '$id'";
	$result = mysqli_query($ms,$query);
